...
import {
  Home, Users, CalendarDays, Wallet, Package, Calculator, Plus, Bell,
  TrendingUp, TrendingDown, Sparkles, Check, X, Trash2, Leaf, Clock,
  ChevronRight, DollarSign, AlertTriangle, Menu, Flower2, PawPrint,
  ShoppingBag, Boxes, PiggyBank, CalendarPlus, UserPlus, Search
} from "lucide-react";
...
const ganapIdeas = [
  {
    ...
    tag: "Content Ganap",
    icon: Sparkles,
    title: "Behind-the-Scenes TikTok Day!",
    desc: "Record a cozy ASMR video packing orders — soft rustling, stamping, and sealing sounds.",
  },
  {
    id: "g2",
    tag: "Sales Event",
    icon: PiggyBank,
    title: "Payday Flash Sale Prep!",
    desc: "Create a limited discount voucher for the upcoming 15th/30th payday rush.",
  },
  {
    id: "g3",
    tag: "Product Strategy",
    icon: PawPrint,
    title: "Student Season Trend!",
    desc: "Design school-bag and backpack friendly keychains for the upcoming school season.",
  },
];
...
const SectionTitle = ({ icon: Icon, title, subtitle }) => (
  <div className="mb-6 flex items-start gap-3">
    <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#E8F0E6] text-[#5F7A5F] shadow-inner shrink-0">
      <Icon size={20} strokeWidth={2.2} />
    </div>
    <div>
      <h2 className="font-display text-[22px] font-semibold text-[#3D3B36] leading-tight">{title}</h2>
      {subtitle && <p className="text-sm text-[#7A776E] mt-0.5">{subtitle}</p>}
    </div>
  </div>
);
...
const Card = ({ children, className = "" }) => (
  <div className={`rounded-3xl bg-white/90 border border-[#EAE6DC] shadow-[0_4px_18px_rgba(90,80,60,0.06)] ${className}`}>
    {children}
  </div>
);
...
const PrimaryButton = ({ children, onClick, icon: Icon, className = "" }) => (
  <button
    onClick={onClick}
    className={`inline-flex items-center gap-1.5 rounded-full bg-[#8FBC8F] px-4 py-2 text-sm font-semibold text-white shadow-sm transition-all hover:bg-[#7CAA7C] hover:shadow-md active:scale-95 ${className}`}
  >
    {Icon && <Icon size={16} />}
    {children}
  </button>
...
const GhostButton = ({ children, onClick, icon: Icon, className = "" }) => (
  <button
    onClick={onClick}
    className={`inline-flex items-center gap-1.5 rounded-full border border-[#D9D2C2] bg-white px-4 py-2 text-sm font-semibold text-[#5C5A52] transition-all hover:bg-[#FAF9F6] active:scale-95 ${className}`}
  >
    {Icon && <Icon size={16} />}
    {children}
  </button>
...
const FieldLabel = ({ children }) => (
  <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-[#8A8677]">{children}</label>
);

const inputCls =
  "w-full rounded-xl border border-[#E3DECF] bg-[#FCFBF8] px-3 py-2 text-sm text-[#3D3B36] outline-none transition focus:border-[#8FBC8F] focus:ring-2 focus:ring-[#8FBC8F]/30";

/* Stock bar with a little climbing leaf — the signature touch */
const StockVine = ({ pct, low }) => (
  <div className="relative w-full">
    <div className="h-2.5 w-full rounded-full bg-[#EDEAE0] overflow-hidden">
      <div
        className={`h-full rounded-full transition-all duration-700 ${low ? "bg-[#D89B6A]" : "bg-[#8FBC8F]"}`}
        style={{ width: `${pct}%` }}
      />
    </div>
    <Leaf
      size={14}
      className={`absolute -top-[7px] -translate-x-1/2 transition-all duration-700 ${low ? "text-[#B4652E]" : "text-[#5F7A5F]"}`}
      style={{ left: `${pct}%` }}
      strokeWidth={2.3}
    />
  </div>
  ...
  return (
    <>
      ...
      >
              ...
              <button
                ...
                className={`group flex w-full items-center gap-3 rounded-2xl px-3.5 py-2.5 text-sm font-medium transition-all ${
                  active
                    ...
                    : "text-[#5C5A52] hover:bg-[#E8F0E6]"
                }`}
              >
                <item.icon size={18} strokeWidth={2.2} className={active ? "text-white" : "text-[#7CA57C]"} />
        ...
        <div className="mt-6 rounded-2xl bg-[#E8F0E6] p-4">
          <p className="text-xs font-semibold text-[#4E6B4E] mb-1 flex items-center gap-1.5">
            <Leaf size={13} /> Molding moods,
          </p>
          <p className="text-xs text-[#6B8E6B] leading-relaxed">one tiny keychain at a time. 🌿</p>
        </div>
      </aside>
    </>
  );
...
/*  Overview Page                                                       */
/* ------------------------------------------------------------------ */
function Overview({ transactions, employees, events, inventory }) {
  const thisMonth = transactions.filter((t) => t.date.startsWith("2026-07"));
  ...
  const nextEvent = useMemo(() => {
    return [...events]
      .filter((e) => e.date !== "TBD" && e.date >= todayISO)
      .sort((a, b) => a.date.localeCompare(b.date))[0];
  }, [events]);

  const lowStockItems = inventory.filter((i) => i.stock / i.max < 0.3);

  const maxChartVal = Math.max(...chartData.map((d) => Math.max(d.revenue, d.expense)));

  const stats = [
    { label: "Total Sales This Month", value: peso(revenue), icon: ShoppingBag, tone: "sage" },
    { label: "Net Profit (Kita)", value: peso(netProfit), icon: PiggyBank, tone: "clay" },
    { label: "Active Employees", value: activeEmployees, icon: Users, tone: "sage" },
    {
      label: "Next Scheduled Event",
      value: nextEvent ? nextEvent.title.split(" ").slice(0, 3).join(" ") + "…" : "None yet",
      sub: nextEvent ? fmtDate(nextEvent.date) : "",
      icon: CalendarDays,
      tone: "clay",
    },
  ];
  ...
  return (
    <div>
      <SectionTitle icon={Home} title="Good day, Moodli! 🌿" subtitle="Here's how the studio is doing this month." />
            ...
            <div className="flex items-center justify-between mb-3">
              <div
                className={`flex h-9 w-9 items-center justify-center rounded-xl ${
                  s.tone === "sage" ? "bg-[#E8F0E6] text-[#5F7A5F]" : "bg-[#F3E3D3] text-[#9C6B3E]"
                }`}
              >
                <s.icon size={17} />
              </div>
            </div>
            <p className="text-xs font-semibold uppercase tracking-wide text-[#9A9689]">{s.label}</p>
            <p className="mt-1 font-display text-xl font-bold text-[#3D3B36] leading-tight">{s.value}</p>
            {s.sub && <p className="text-xs text-[#B4652E] font-medium mt-0.5">{s.sub}</p>}
          </Card>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <Card className="xl:col-span-2 p-6">
          <div className="flex items-center justify-between mb-5">
            <div>
              <p className="font-display text-base font-semibold text-[#3D3B36]">Revenue vs. Expenses</p>
              <p className="text-xs text-[#9A9689]">Last 6 months, in ₱</p>
            </div>
            <div className="flex items-center gap-3 text-xs font-medium">
              <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-[#8FBC8F]" /> Revenue</span>
              <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-[#D89B6A]" /> Expenses</span>
            </div>
          </div>
